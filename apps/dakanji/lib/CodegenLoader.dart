...
class CodegenLoader extends AssetLoader{
  ...
  "SettingsScreen": {
    ...
    "anki_allow_duplicates_explanation": "通过比较 DaKanji 注释类型的第一个字段来检查重复。在该字段中具有完全相同内容的所有其他卡片都是匹配的。\n默认情况下，这是翻译字段。因此，如果您修改了翻译，就可以再次添加相同的卡片。不过，您可以在 Anki 中更改字段的顺序。当使用 \"DaKanji link（大汉字链接）\"字段作为第一个字段时，每个字典条目都是唯一标识的。",
    ...
    "dict_base_form_description": "这个设置使动词、形容词和副词的 \"去伪存真 \"成为可能。当它被启用时，DaKanji会尝试将搜索词转换成其基本形式。例如，食べます、食べられる、食べない都被转换为食べる。\n对于中等水平的学习者，建议关闭这个设置。",
    "dict_convert_to_kana": "转换为假名的术语",
    "dict_convert_to_kana_description": "当启用此设置时，DaKanji会在查找前将任何搜索词转换为统一的假名表示。这意味着查询 \"taberu \"将被转换为たべる，并能找到食べる，但这也可能导致非预期的匹配。\n\n例如，搜索'man'会被转换为まん，因此会匹配到万和英文单词'man'。\n\n注意：这是一个全局设置，在使用词典的任何地方都适用",
    ...
    "dict_search_sorting_information_description_1": "该设置允许你通过改变排序优先级（拖动以重新排序）或禁用搜索词修改（轻点以禁用）来自定义达坎吉如何对词典搜索结果进行排序。\n为了充分理解这一设置，让我们以搜索词 \"shikunshi\"（注意，它是用罗马字书写的）为例，看看 DaKanji 如何对搜索结果进行排序。DaKanji 将根据以下标准显示搜索结果，其中较深的嵌套标准的重要性低于较高的标准。",
...
static const Map<String,dynamic> _ja = {
  ...
  "ClipboardScreen": {
    ...
    "tutorial_explanation": "テキストをコピーすると、Dakanji上で辞書の項目等を表示します！",
    "tutorial_pin_button": "このボタンを押すと、Dakanjiが他のウィンドウの上に常に表示されるようになります。また、アプリのサイズが小さくなり、他のアプリケーション上で読書補助として使用できます。"
  },
  "DictionaryScreen": {
    "example_tab": "例",
    "examples_more": "その他の例",
  ...
  "InitScreen": {
    ...
    "complete_log": "全てのログ",
    "downgrade_warning": "旧バージョンのDakanjiを使用しています！保存されているデータが破損する可能性があります！「設定」→「詳細設定」で全てのデータをリセットし、アプリを再起動してください",
  ...
  "ManualScreen": {
    "anki_connection_test_text": "DaKanjiとAnkiを使い始めるには、下のボタンを押してください。設定が正しいかどうかチェックできます。\n\n成功のメッセージが表示された場合、DaKanjiとAnkiは正しく接続され、使い始めることができます。\nエラーメッセージが表示された場合、メッセージの指示に従って、再度ボタンを押してください。\n成功メッセージが表示されないと、DaKanjiからAnkiにカードを送信できません。以下のボタンを押して、成功メッセージが表示されることを確認してください。設定が成功したかどうかは、設定→Ankiでも確認できます。",
    ...
    "anki_test_connection_note_type_not_available": "DaKanjiのノートタイプがAnkiに存在しません！iOSの設定ガイドで記載されている既成デッキをインポートして手動で追加できます。",
    ...
    "misc_drawer_reorder_text": "DaKanjiでは、すべてのユーザーに最適な方法でご利用いただけるよう、メインメニューの順番の変更が可能です。メニュー項目をドラッグして好きな場所に移動できます。",
    "misc_title": "その他",
  ...
  "SettingsScreen": {
    ...
    "dict_base_form_description": "この設定は、動詞、形容詞、助動詞の「非活用」を有効にします。これが有効になると、DaKanjiは検索語を基本形に変換しようとします。例えば、「食べます」、「食べられる」、「食べない」はすべて「食べる」に変換されます。\n\n中級者には、この設定をオフにすることをお勧めします。",
    "dict_convert_to_kana": "かな変換された用語",
    "dict_convert_to_kana_description": "この設定を有効にすると、DaKanjiは検索語を統一された仮名表記に変換してから検索を行います。たとえば、検索語「taberu」は「たべる」に変換され、「食べる」を見つけることができますが、意図しないマッチを引き起こす可能性もあります。\n\n例えば、「man」を検索すると「まん」に変換されるため、「万」や英単語「man」などにマッチすることがあります。\n\n注意: この設定はグローバル設定であり、辞書が使用されるすべての場所に適用されます。",
    "dict_custom_query_format_body": "DaKanjiは、辞書の画像検索の検索語を現在の辞書項目に置き換えます。このプレースホルダーは「{kanjiPlaceholder}」です。\n\n例：\n現在の単語が「口」で、「口 意味」という検索語で画像を検索したい場合、このフィールドに「{kanjiPlaceholder}意味」と入力してください。",
    ...
    "dict_show_word_freq_body": "DaKanjiは、単語の使用頻度を利用して、辞書項目のランク付けを行います。これは単語の使用頻度を推定したもので、さまざまなデータソースに基づくものです：\n\n* ウィキペディア\n* OPUS OpenSubtitles 2018およびSUBTLEXの字幕\n* ウェブテキスト、OSCAR\n* Twitter\n* Reddit\n\nDaKanjiで表示される数字は、実際の単語頻度のバリエーションである[Zipf frequency](https://en.wikipedia.org/wiki/Zipf%27s_law)に基づいています。Zipf頻度は、人間にとって理解しやすい対数スケールで単語の頻度を表すためのものです。Zipf頻度は、単語が1億語あたりに出現する回数の常用対数です。例えば、Zipf値が6の単語は1千語ごとに1回出現し、Zipf値が3の単語は1百万語ごとに1回出現します。\n\n合理的なZipf値は0から8の範囲ですが、DaKanjiに表示される最低のZipf値は1.0です。データソースに存在しない単語には0がデフォルトのZipf値として使用されますが、これは1億語あたりに1回の出現を意味します。\n[もっと読む...](https://github.com/rspeer/wordfreq)",
    "dict_term": "検索用語",
...
static const Map<String,dynamic> _de = {
  ...
  "ManualScreen": {
    ...
    "anki_test_connection_note_type_not_available": "Der DaKanji-Notiztyp ist in Anki nicht verfügbar! Sie können ihn manuell hinzufügen, indem Sie das vorgefertigte Deck importieren, das in der iOS-Einrichtungsanleitung erwähnt wird.",
  ...
  "SettingsScreen": {
    ...
    "anki_allow_duplicates_explanation": "Duplikate werden geprüft, indem das erste Feld des DaKanji-Notiztyps verglichen wird. Eine Übereinstimmung ist jede andere Karte, die genau den gleichen Inhalt in diesem Feld hat.\nStandardmäßig ist dies das Übersetzungsfeld. Wenn Sie also die Übersetzung ändern, können Sie die gleiche Karte wieder hinzufügen. Sie können jedoch die Reihenfolge der Felder in Anki ändern. Jeder Wörterbucheintrag wird eindeutig identifiziert, wenn das Feld \"DaKanji link\" als erstes Feld verwendet wird.",
    ...
    "dict_search_sorting_information_description_1": "Mit dieser Einstellung können Sie festlegen, wie Dakanji die Suchergebnisse von Wörterbüchern sortiert, indem Sie die Sortierpriorität ändern (ziehen, um neu zu sortieren) oder die Änderung von Suchbegriffen deaktivieren (tippen, um sie zu deaktivieren).\\\nUm diese Einstellung vollständig zu verstehen, schauen wir uns an, wie DaKanji die Suchergebnisse sortiert, indem wir den Beispielsuchbegriff \"shikunshi\" verwenden (beachten Sie, dass er in romaji geschrieben ist). DaKanji zeigt die Suchergebnisse anhand der folgenden Kriterien an, wobei tiefer verschachtelte Kriterien weniger wichtig sind als höhere.",
...
static const Map<String,dynamic> _ru = {
  ...
  "ManualScreen": {
    ...
    "anki_test_connection_note_type_not_available": "Тип нот DaKanji недоступен в Anki! Вы можете добавить его вручную, импортировав готовую колоду, о которой говорится в руководстве по настройке iOS.",
  ...
  "SettingsScreen": {
    ...
    "dict_base_form_description": "Этот параметр включает 'деконъюгацию' глаголов, прилагательных и копул. Когда она включена, DaKanji пытается преобразовать поисковый термин в его базовую форму. Например, 食べます、食べられる、食べない все преобразуется в 食べる.\nДля учащихся среднего уровня рекомендуется отключить эту настройку.",
    "dict_convert_to_kana": "Термин, преобразованный в кану",
    "dict_convert_to_kana_description": "При включении этой настройки DaKanji преобразует любой поисковый запрос в унифицированное представление каны, прежде чем искать его. Это означает, что запрос 'taberu' будет преобразован в たべる и будет найден 食べる, но это также может привести к непреднамеренным совпадениям.\n\nНапример, запрос 'man' будет преобразован в まん и, следовательно, найдет 万 и английское слово 'man'.\n\nПримечание: это глобальная настройка, которая применяется везде, где используется словарь",
...
static const Map<String,dynamic> _pl = {
  ...
  "ManualScreen": {
    ...
    "anki_test_connection_note_type_not_available": "Typ notatki DaKanji nie jest dostępny w Anki! Możesz dodać go ręcznie, importując gotową talię wspomnianą w przewodniku konfiguracji iOS.",
  ...
  "SettingsScreen": {
    ...
    "anki_allow_duplicates_explanation": "Duplikaty są sprawdzane poprzez porównanie pierwszego pola typu notatki DaKanji. Dopasowaniem jest każda inna karta, która ma dokładnie taką samą zawartość w tym polu.\nDomyślnie jest to pole tłumaczenia. Dlatego jeśli zmodyfikujesz tłumaczenie, dodanie tej samej karty będzie możliwe ponownie. Można jednak zmienić kolejność pól w Anki. Każdy wpis słownika jest jednoznacznie identyfikowany przy użyciu pola \"DaKanji link\" jako pierwszego pola.",
    ...
    "dict_base_form_description": "To ustawienie umożliwia 'dekonjugację' czasowników, przymiotników i copula. Kiedy jest włączone, DaKanji próbuje przekonwertować szukany termin na jego formę podstawową. Na przykład, 食べます、食べられる、食べない wszystkie są konwertowane na 食べる.\nDla średnio zaawansowanych zalecane jest wyłączenie tego ustawienia.",
    "dict_convert_to_kana": "Termin przekonwertowany na kana",
    "dict_convert_to_kana_description": "Po włączeniu tego ustawienia, DaKanji konwertuje każde wyszukiwane słowo na jednolitą reprezentację kana przed wyszukaniem go. Oznacza to, że zapytanie 'taberu' zostanie przekonwertowane na たべる i znajdzie 食べる, ale może to również prowadzić do niezamierzonych dopasowań.\n\nNa przykład wyszukiwanie 'man' zostanie przekonwertowane na まん i dlatego dopasuje 万 i angielskie słowo 'man'.\n\nUwaga: jest to ustawienie globalne i jest stosowane wszędzie tam, gdzie słownik jest używany",
    ...
    "dict_search_sorting_information_description_1": "To ustawienie pozwala dostosować sposób, w jaki Dakanji sortuje wyniki wyszukiwania słownika, zmieniając priorytet sortowania (przeciągnij, aby zmienić kolejność) lub wyłączając modyfikację wyszukiwanego hasła (dotknij, aby wyłączyć).\nAby w pełni zrozumieć to ustawienie, przyjrzyjmy się, jak DaKanji sortuje wyniki wyszukiwania przy użyciu przykładowego wyszukiwanego hasła \"shikunshi\" (zwróć uwagę, że jest ono zapisane w romaji). DaKanji wyświetli wyniki wyszukiwania w oparciu o następujące kryteria, gdzie głębiej zagnieżdżone kryteria są mniej ważne niż wyższe.",
...
static const Map<String,dynamic> _en = {
  ...
  "ManualScreen": {
    ...
    "anki_test_connection_note_type_not_available": "The DaKanji note type is not available in Anki! You can add it manually by importing the premade deck mentioned in the iOS setup guide.",
  ...
  "SettingsScreen": {
    ...
    "anki_allow_duplicates_explanation": "Duplicates are checked by comparing the first field of the DaKanji note type. A match is every other card that has the exact same content in that field.\nBy default, this is the translation field. Therefore, if you modify the translation, adding the same card will be possible again. However, you can change the order of the fields in Anki. Every dictionary entry is uniquely identified when using the \"DaKanji link\" field as the first field.",
    ...
    "dict_base_form_description": "This setting enables the 'deconjugation' of verbs, adjectives, and copula. When it is enabled, DaKanji tries to convert the search term into its base form. For example, 食べます、食べられる、食べない all are converted into 食べる.\n\nFor intermediate learners, the recommendation is to turn off this setting.",
    "dict_convert_to_kana": "Term converted to kana",
    "dict_convert_to_kana_description": "When enabling this setting, DaKanji converts any search term to a uniform kana representation before looking it up. This means that the query 'taberu' will be converted to たべる and will find 食べる, but it can also lead to unintended matches.\n\nFor example searching 'man' will be converted to まん and therfore, matches 万 and the english word 'man'.\n\nNote: this is a global setting and is applied everywhere the dictionary is used",
    ...
    "dict_search_sorting_information_description_1": "This setting allows you to customize how Dakanji sorts dictionary search results by changing the sort priority (drag to reorder) or disabling search term modification (tap to disable).\\\nTo fully understand this setting, let us look at how DaKanji sorts search results using the example search term \"shikunshi\" (note that it is written in romaji). DaKanji will display search results based on the following criteria where deeper nested criteria are less important than higher ones.",
...
static const Map<String,dynamic> _it = {
  ...
  "ManualScreen": {
    ...
    "anki_test_connection_note_type_not_available": "Il tipo di nota DaKanji non è disponibile in Anki! È possibile aggiungerlo manualmente importando il mazzo preconfezionato menzionato nella guida alla configurazione di iOS.",
  ...
  "SettingsScreen": {
    ...
    "dict_base_form_description": "Questa impostazione abilita la \"deconiugazione\" di verbi, aggettivi e copule. Quando è abilitata, DaKanji cerca di convertire il termine di ricerca nella sua forma base. Ad esempio, 食べます、食べられる、食べない tutti sono convertiti in 食べる.\nPer gli studenti di livello intermedio, si consiglia di disattivare questa impostazione.",
    "dict_convert_to_kana": "Termine convertito in kana",
    "dict_convert_to_kana_description": "Se si attiva questa impostazione, DaKanji converte qualsiasi termine di ricerca in una rappresentazione kana uniforme prima di cercarlo. Ciò significa che la query \"taberu\" sarà convertita in たべる e troverà 食べる, ma può anche portare a corrispondenze non volute.\n\nAd esempio, la ricerca di \"man\" verrà convertita in まん e quindi corrisponderà a 万 e alla parola inglese \"man\".\n\nNota: questa è un'impostazione globale e viene applicata ovunque venga utilizzato il dizionario.",
    ...
    "dict_search_sorting_information_description_1": "Questa impostazione consente di personalizzare il modo in cui Dakanji ordina i risultati della ricerca nei dizionari, cambiando la priorità dell'ordinamento (trascinare per riordinare) o disabilitando la modifica dei termini di ricerca (toccare per disabilitare).\nPer comprendere appieno questa impostazione, vediamo come DaKanji ordina i risultati della ricerca utilizzando il termine di esempio \"shikunshi\" (notare che è scritto in romaji). DaKanji visualizza i risultati della ricerca in base ai seguenti criteri, dove i criteri più profondi sono meno importanti di quelli più alti.",
...
static const Map<String,dynamic> _fr = {
  ...
  "ManualScreen": {
    ...
    "anki_test_connection_note_type_not_available": "Le type de note DaKanji n'est pas disponible dans Anki ! Vous pouvez l'ajouter manuellement en important le jeu de notes pré-fabriqué mentionné dans le guide d'installation iOS.",
  ...
  "SettingsScreen": {
    ...
    "anki_allow_duplicates_explanation": "Les doublons sont vérifiés en comparant le premier champ du type de note DaKanji. Une correspondance est une carte sur deux qui a exactement le même contenu dans ce champ.\nPar défaut, il s'agit du champ de traduction. Par conséquent, si vous modifiez la traduction, il sera à nouveau possible d'ajouter la même carte. Vous pouvez toutefois modifier l'ordre des champs dans Anki. Chaque entrée du dictionnaire est identifiée de manière unique en utilisant le champ \"DaKanji link\" comme premier champ.",
    ...
    "dict_base_form_description": "Ce paramètre active la \"déconjugaison\" des verbes, des adjectifs et des copules. Lorsqu'il est activé, DaKanji essaie de convertir le terme de recherche dans sa forme de base. Par exemple, 食べます、食べられる、食べない all est converti en 食べる.\nPour les apprenants de niveau intermédiaire, il est recommandé de désactiver ce paramètre.",
    "dict_convert_to_kana": "Terme converti en kana",
    "dict_convert_to_kana_description": "Lorsque ce paramètre est activé, DaKanji convertit tout terme de recherche en une représentation kana uniforme avant de le rechercher. Cela signifie que la requête 'taberu' sera convertie en たべる et trouvera 食べる, mais cela peut également conduire à des correspondances inattendues.\n\nPar exemple, la recherche \"man\" sera convertie en まん et correspondra donc à 万 et au mot anglais \"man\".\n\nRemarque : il s'agit d'un paramètre global qui s'applique partout où le dictionnaire est utilisé.",
    ...
    "dict_search_sorting_information_description_1": "Ce paramètre vous permet de personnaliser la façon dont Dakanji trie les résultats de recherche dans les dictionnaires en modifiant la priorité de tri (faites glisser pour réorganiser) ou en désactivant la modification des termes de recherche (touchez pour désactiver).\nPour bien comprendre ce paramètre, regardons comment DaKanji trie les résultats de recherche en utilisant l'exemple du terme de recherche \"shikunshi\" (notez qu'il est écrit en romaji). DaKanji affichera les résultats de la recherche en fonction des critères suivants, les critères les plus profonds étant moins importants que les plus élevés.",
...
static const Map<String,dynamic> _es = {
  ...
  "ManualScreen": {
    ...
    "anki_test_connection_note_type_not_available": "¡El tipo de nota DaKanji no está disponible en Anki! Puedes añadirlo manualmente importando el mazo prefabricado mencionado en la guía de configuración de iOS.",
  ...
  "SettingsScreen": {
    ...
    "anki_allow_duplicates_explanation": "Los duplicados se comprueban comparando el primer campo del tipo de nota DaKanji. Una coincidencia es cualquier otra tarjeta que tenga exactamente el mismo contenido en ese campo.\nPor defecto, este es el campo de traducción. Por lo tanto, si modifica la traducción, será posible volver a añadir la misma ficha. Sin embargo, puedes cambiar el orden de los campos en Anki. Cada entrada del diccionario se identifica de forma única cuando se utiliza el campo \"Enlace DaKanji\" como primer campo.",
    ...
    "dict_base_form_description": "Esta opción activa la \"desconjugación\" de verbos, adjetivos y cópulas. Cuando está activada, DaKanji intenta convertir el término buscado en su forma base. Por ejemplo, 食べます、食べられる、食べない se convierte en 食べる.\nPara los alumnos de nivel intermedio, la recomendación es desactivar este ajuste.",
    "dict_convert_to_kana": "Término convertido a kana",
    "dict_convert_to_kana_description": "Al activar esta opción, DaKanji convierte cualquier término de búsqueda a una representación kana uniforme antes de buscarlo. Esto significa que la consulta \"taberu\" se convertirá a たべる y encontrará 食べる, pero también puede dar lugar a coincidencias no deseadas.\n\nPor ejemplo, al buscar \"man\" se convertirá en まん y, por lo tanto, coincidirá con 万 y la palabra inglesa \"man\".\n\nNota: esta configuración es global y se aplica en todos los lugares en los que se utiliza el diccionario.",
    ...
    "dict_search_sorting_information_description_1": "Este ajuste te permite personalizar la forma en que Dakanji ordena los resultados de la búsqueda en el diccionario cambiando la prioridad de ordenación (arrastra para reordenar) o desactivando la modificación del término de búsqueda (toca para desactivar).\\\nPara entender completamente esta opción, veamos cómo DaKanji ordena los resultados de búsqueda utilizando el término de búsqueda de ejemplo \"shikunshi\" (ten en cuenta que está escrito en romaji). DaKanji mostrará los resultados de la búsqueda basándose en los siguientes criterios, en los que los criterios anidados más profundos son menos importantes que los superiores.",
